...
export default function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    getPost(slug).then((data) => {
      if (!data) {
        // Handle 404 client-side or just show not found state
      }
      setPost(data);
      setLoading(false);
    });
        ...
        <div className="prose prose-lg max-w-none font-serif leading-relaxed">
            <ReactMarkdown 
                ...
                components={{
                    ...
                    a: ({node, ...props}) => <a {...props} className="text-primary hover:underline underline-offset-4" target="_blank" rel="noopener noreferrer" />
                }}
            >
                {post.content}
            </ReactMarkdown>
